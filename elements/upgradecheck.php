	...
	var   $_name = 'Upgradecheck';
	  ...
	  function fetchElement($name, $value, &$node, $control_name)
	  {
		...
		//If cURL is supported, check the current version available.
				...
				$version = 0.1;
				$target = 'http://joomlaengineering.com/upgradecheck/construct5-5';
				$curl = curl_init();
				...
				$message = 'You are using Construct5 version 1.5.'.$version.'. ';
				
				//If the current version is out of date, notify the user and provide a download link.
				if ($version < $str)
					$message = $message . '<a href="http://joomlaengineering.com" target="_blank">Version 1.5.'.$str.' is now available.</a><br /><a href="http://joomlaengineering.com/construct5-5-changelog" target="_blank">See what&rsquo;s new</a>.';
				//If the current version is up to date, notify the user. 	
				elseif (($version == $str) || ($version > $str))
					$message = $message . 'There are no updates available at this time.<br /><a href="http://joomlaengineering.com/construct5-5-changelog" target="_blank">View the change log</a>.';
				return $message;							
	  }
}